    ...
    return new Promise((resolve, reject) => {
      ...
      this.watcher.on('event', event => {
        if (event.code === 'START') {
          ...
          this.buildId = isPending ? bId : ++buildId;
          this.eventBus.emit('start', this.type, this.buildId);
          ...
          isPending = false;
          resolve();
        } else if (event.code === 'FATAL' || event.code === 'ERROR') { // 'ERROR'
          const err = JSON.stringify(event.error, void 0, 2) + '\n' + event.error.toString();
          const id = this.buildId;
          this.buildId = null;
          !isPending && this.eventBus.emit('error', new ErrorInfo(err, { file: 'all', type: 'rollup' }), id);
