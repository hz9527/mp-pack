 ...
 * @param
 {
  entry: string | {
    ...
    ext: {js: string | string[], css: string | string[], xml: string | string[], json: string | string[]} // default js scss wxml json
  },
  output: string | {
    path: string,
    name: string, // default is app
  ...
  if (conf.ext) {
    Object.keys(ext).forEach(key => {
      if (typeof conf.ext[key] === 'string' ||
       (isEntry && Array.isArray(conf.ext[key]) && conf.ext[key].every(e => typeof e === 'string'))) {
        ext[key] = conf.ext[key];
      } else {
        conf.ext[key] && error({
          code: 'EXT_IS_INVAILD',
          message: isEntry ?
            `${JSON.stringify(conf.ext[key])} is not string, or include item isn't string` 
            : `${conf.ext[key]} is not string`
        })
      }
    });
  }
